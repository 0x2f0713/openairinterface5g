...
uint8_t do_MIB_NR(gNB_RRC_INST *rrc, uint32_t frame)
{
  ...
  // encode MIB to data
  //  printf("0x2f0713\n");
  xer_fprint(stdout, &asn_DEF_NR_BCCH_BCH_Message, (void *)mib);
                   ...
                   gNB_RrcConfigurationReq *configuration)
{
  ...
  */
  ...
  if (carrier->SIB1 == NULL)
    carrier->SIB1 = (uint8_t *)malloc16(NR_MAX_SIB_LENGTH / 8);
...
//------------------------------------------------------------------------------
{
  ...
  dl_dcch_msg.message.choice.c1->choice.securityModeCommand->criticalExtensions.choice.securityModeCommand->securityConfigSMC.securityAlgorithmConfig.integrityProtAlgorithm = integrityProtAlgorithm;
...
//------------------------------------------------------------------------------
{
  ...
  dl_dcch_msg.message.choice.c1->choice.dlInformationTransfer->criticalExtensions.choice.dlInformationTransfer = CALLOC(1, sizeof(NR_DLInformationTransfer_IEs_t));
  dl_dcch_msg.message.choice.c1->choice.dlInformationTransfer->criticalExtensions.choice.dlInformationTransfer->dedicatedNAS_Message = CALLOC(1, sizeof(NR_DedicatedNAS_Message_t));
...
uint8_t
do_NR_CounterCheck(uint8_t *buffer, size_t buffer_size,
                   uint8_t Transaction_id)
{
  asn_enc_rval_t enc_rval;
  NR_DL_DCCH_Message_t dl_dcch_msg;
  NR_CounterCheck_t *nrCounterCheck;


  memset((void *)&dl_dcch_msg, 0, sizeof(NR_DL_DCCH_Message_t));
  dl_dcch_msg.message.present = NR_DL_DCCH_MessageType_PR_c1;
  dl_dcch_msg.message.choice.c1 = calloc(1, sizeof(struct NR_DL_DCCH_MessageType__c1));
  dl_dcch_msg.message.choice.c1->present = NR_DL_DCCH_MessageType__c1_PR_counterCheck;
  dl_dcch_msg.message.choice.c1->choice.counterCheck = CALLOC(1, sizeof(NR_CounterCheck_t));

  nrCounterCheck = dl_dcch_msg.message.choice.c1->choice.counterCheck;

  nrCounterCheck->rrc_TransactionIdentifier = Transaction_id;
  // nrCounterCheck->criticalExtensions.present = NR_CounterCheck__criticalExtensions_PR_criticalExtensionsFuture;
  nrCounterCheck->criticalExtensions.present = NR_CounterCheck__criticalExtensions_PR_counterCheck;
  nrCounterCheck->criticalExtensions.choice.counterCheck = CALLOC(1, sizeof(NR_CounterCheck_IEs_t));
  
  // asn1cSequenceAdd(nrCounterCheck->criticalExtensions.choice.counterCheck->drb_CountMSB_InfoList.list, NR_DRB_CountMSB_Info_t, drbCountMSBInfo);
  NR_CounterCheck_IEs_t *counterCheckIEs = nrCounterCheck->criticalExtensions.choice.counterCheck;
  counterCheckIEs->lateNonCriticalExtension = NULL;
  counterCheckIEs->nonCriticalExtension     = NULL;
  NR_DRB_CountMSB_Info_t *drbCountMSBInfo = calloc(1, sizeof(NR_DRB_CountMSB_Info_t));
  drbCountMSBInfo->drb_Identity = 1;
  drbCountMSBInfo->countMSB_Uplink = 9;
  drbCountMSBInfo->countMSB_Downlink = 10;
  ASN_SEQUENCE_ADD(&(counterCheckIEs->drb_CountMSB_InfoList.list), drbCountMSBInfo);
  if (LOG_DEBUGFLAG(DEBUG_ASN1))
  {
    printf("CounterCheck: \n");
    xer_fprint(stdout, &asn_DEF_NR_DL_DCCH_Message, (void *)&dl_dcch_msg);
  ...
  LOG_D(NR_RRC, "CounterCheck Encoded %u bits (%u bytes)\n",
        (uint32_t)enc_rval.encoded, (uint32_t)(enc_rval.encoded + 7) / 8);
  return ((enc_rval.encoded + 7) / 8);
}

uint8_t do_NR_CounterCheckResponse(
                  uint8_t *buffer, size_t buffer_size,
                  uint8_t Transaction_id) {
  asn_enc_rval_t enc_rval;
  NR_UL_DCCH_Message_t ul_dcch_msg;
  NR_CounterCheckResponse_t *nrCounterCheckResponse; 


  memset((void *)&ul_dcch_msg, 0, sizeof(NR_UL_DCCH_Message_t));
  ul_dcch_msg.message.present = NR_UL_DCCH_MessageType_PR_c1;
  ul_dcch_msg.message.choice.c1 = calloc(1, sizeof(struct NR_DL_DCCH_MessageType__c1));
  ul_dcch_msg.message.choice.c1->present = NR_UL_DCCH_MessageType__c1_PR_counterCheckResponse;
  ul_dcch_msg.message.choice.c1->choice.counterCheckResponse = CALLOC(1, sizeof(NR_CounterCheckResponse_t));

  nrCounterCheckResponse = ul_dcch_msg.message.choice.c1->choice.counterCheckResponse;
  nrCounterCheckResponse->rrc_TransactionIdentifier = Transaction_id;
  nrCounterCheckResponse->criticalExtensions.present = NR_CounterCheckResponse__criticalExtensions_PR_counterCheckResponse;
  nrCounterCheckResponse->criticalExtensions.choice.counterCheckResponse = calloc(1, sizeof(NR_CounterCheckResponse_IEs_t));

  NR_CounterCheckResponse_IEs_t *nrCounterCheckResponseIEs = nrCounterCheckResponse->criticalExtensions.choice.counterCheckResponse;
  nrCounterCheckResponseIEs->lateNonCriticalExtension = NULL;
  nrCounterCheckResponseIEs->nonCriticalExtension     = NULL;

  NR_DRB_CountInfo_t *drbCountInfo = calloc(1, sizeof(NR_DRB_CountInfo_t));
  drbCountInfo->drb_Identity = 1;
  drbCountInfo->count_Uplink = 2;
  drbCountInfo->count_Downlink = 3;
  ASN_SEQUENCE_ADD(&(nrCounterCheckResponseIEs->drb_CountInfoList.list), drbCountInfo);

  if (LOG_DEBUGFLAG(DEBUG_ASN1))
  {
    printf("CounterCheckResponse: \n");
    xer_fprint(stdout, &asn_DEF_NR_UL_DCCH_Message, (void *)&ul_dcch_msg);
  }
  ...
  LOG_D(NR_RRC, "CounterCheckResponse Encoded %u bits (%u bytes)\n",
        (uint32_t)enc_rval.encoded, (uint32_t)(enc_rval.encoded + 7) / 8);
  return ((enc_rval.encoded + 7) / 8);
}